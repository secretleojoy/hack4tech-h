...
def gen_result_infos(items, infos, subdomains, appear_times, wc_ips, wc_ttl):
    ...
    for answer in answers:
        ...
        ttl = answer.get('ttl')
        ttls.append(ttl)
        name = answer.get('name')  # Remove the rightmost `.` dot
    ...
    # In order to optimize the memory, only add the subdomains that have the A record and pass the judgment to the record
    ...
    with open(result_path) as fd:
        for line in fd:
            ...
            for answer in answers:
                if answer.get('type') == 'A':
                    ip = answer.get('data')
                    # Value If it is the IP set that appears for the first time, the number of occurrences is assigned to 0 first
...
def deal_output(output_path, appear_times, wildcard_ips, wildcard_ttl):
    ...
    subdomains = list()  # Used to save all subdomains that pass the validity check
...
class Brute(Module):
    ...
    def main(self, domain):
        ...
        wildcard_ips = list()  # Pan-analysis IP list
        wildcard_ttl = int()  # Pan-analysis of TTL integer values
        ns_list = query_domain_ns(self.domain)
        ns_ip_list = query_domain_ns_a(ns_list)  # DNS authoritative name server corresponding A record list
