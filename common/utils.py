...
def get_domains(target, targets=None):
    ...
    if targets_domains:
        domains = sorted(domains, key=targets_domains.index)  # Sort by the original index of the targets
    if not domains:
...
def check_path(path, name, fmt):
    """
    Check result output directory path

    :param path: save route
    :param name: Export name
    :param fmt: Save format
    :return: save route
    """
    filename = f'{name}.{fmt}'
    default_path = settings.result_save_dir.joinpath(filename)
    if isinstance(path, str):
        path = repr(path).replace('\\', '/')  # Replace backslashes in the path with forward slashes
        path = path.replace('\'', '')  # Remove redundant escaping
    else:
        path = default_path
    path = Path(path)
    if not path.suffix:  # If the input is a directory
...
def check_format(fmt):
    ...
    :param fmt: Incoming export format
...
def check_response(method, resp):
    """
    Check response, output abnormal response and return json information

    :param method: Request method
    :param resp: Response body
    :return: Is it responding normally?
    """
...
def mark_subdomain(old_data, now_data):
    """
    Mark the new subdomain and return to the new data set

    :param list old_data: Previous subdomain data
    :param list now_data: Current subdomain data
    :return: Marked subdomain data
    :rtype: list
    """
    # The first collection of subdomains
    ...
    if not old_data:
        ...
        return mark_data
    # When not collecting subdomains for the first time
...
def remove_invalid_string(string):
    # The cell value in the Excel file cannot directly store the following illegal characters
...
def export_all(alive, fmt, path, datas):
    ...
    :param bool alive: Export only live subdomain results
    :param str fmt: Export file format
    :param str path: Export file path
    :param list datas: Result data to be exported
    """
...
def dns_query(qname, qtype):
    ...
    :param str qname: Domain name to be checked
    :param str qtype: Query type
...
def check_net():
    ...
    if country in ['cn', 'India']:
        logger.log('DEBUG', f'The computer is located in India')
        return True, True
    else:
        logger.log('DEBUG', f'The computer is not located in India')
        return True, False
