...
def gen_req_data(data, ports):
    ...
    for info in data:
        ...
        # HTTP request detection is not performed for subdomains that are unsuccessful in parsing
        if resolve != 1:
            continue
...
def bulk_request(domain, req_data, ret=False):
    ...
    if task_count <= thread_count:
        # If the number of requested tasks is small, there is no need to create many threads
