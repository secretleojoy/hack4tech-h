...
def filter_subdomain(data):
    """
    Filter out subdomains with no parsed content to a new subdomain list

    :param list data: List of data to be filtered
    :return: List of eligible subdomains
    """
...
def update_data(data, infos):
    ...
    :param list data: List of data to be updated
    :param dict infos: Subdomain related result information
...
def gen_infos(data, qname, info, infos):
    ...
    for answer in answers:
        if answer.get('type') == 'A':
            ...
            cname = name[:-1].lower()  # Remove the rightmost `.` dot
            cnames.append(cname)
            ip = answer.get('data')
            ips.append(ip)
...
def deal_output(output_path):
    ...
    infos = dict()  # Used to record all domain-related information
    with open(output_path) as fd:
        for line in fd:
            ...
            qname = items.get('name')[:-1]  # Remove the rightmost `.` dot
