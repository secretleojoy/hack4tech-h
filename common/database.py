...
class Database(object):
    ...
    def get_conn(db_path):
        ...
        if isinstance(db_path, Connection):
            return db_path
        protocol = 'sqlite:///'
        if not db_path:  # The database path is empty to connect to the default database
            db_path = f'{protocol}{settings.result_save_dir}/result.sqlite3'
        else:
            db_path = f'{protocol}{db_path}'
        db = records.Database(db_path)  # A new database will be created when there is no database
